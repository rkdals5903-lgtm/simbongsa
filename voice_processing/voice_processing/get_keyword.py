...
class GetKeyword(Node):
    ...
    def main_loop(self):
        ...
        if self.current_mode == "WAKEUP":
            ...
            # 쿨타임 체크
            if time.time() - self.last_command_time < self.cooldown_duration:
                return

            self.get_logger().info("시동어 감지! 음성 인식을 시작합니다.")
            
            # 분석 중에는 중복 감지 방지를 위해 SLEEP 모드로 전환
            self.current_mode = "SLEEP" 

            # [핵심] mic_controller의 스트림을 그대로 공유해서 사용
            ...
            if objects and targets:
                ...
                self.get_logger().info("명령을 이해하지 못했습니다. 다시 시동어를 불러주세요.")
                # 실패 시 다시 시동어 대기 모드로 복귀
                self.current_mode = "WAKEUP"
