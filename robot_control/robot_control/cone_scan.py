    ...
    def scan(self, center_pose, target_name):
        ...
        def result_callback(future):
            try:
                ...
                if result.found:
                    result_data["coordinate"] = list(result.coordinate)

                result_data["message"] = result.message

                node.get_logger().info(f"YOLO 결과: found={result.found}, message={result.message}")
                ...
                node.get_logger().error(f"YOLO 결과 처리 오류: {error}")
        ...
        def goal_callback(future):
            try:
                ...
                result_data["accepted"] = (goal_handle.accepted)

                if goal_handle.accepted:
                    node.get_logger().info(f"탐색 요청 승인: {target_name}")

                    result_future = (goal_handle.get_result_async())
                    result_future.add_done_callback(result_callback)
                else:
                    node.get_logger().warning("탐색 요청이 거절되었습니다.")

            except Exception as error:
                node.get_logger().error(f"Goal 처리 오류: {error}")

            finally:
                goal_event.set()

        if center_pose is None or len(center_pose) != 6:
            node.get_logger().error("스캔 중심 좌표가 올바르지 않습니다.")
            return None
        ...
        for index in range(self.scan_point_count):
            theta = (2.0 * math.pi * index / self.scan_point_count)

            tilt_rx = (self.scan_tilt_angle * math.cos(theta))
            tilt_ry = (self.scan_tilt_angle * math.sin(theta))
            pose = [
                center_pose[0],
                center_pose[1],
                center_pose[2],
        ...
        if not goal_event.wait(timeout=5.0):
            node.get_logger().error("YOLO Goal 응답 시간 초과")
            return None

        if not result_data["accepted"]:
            return None

        node.get_logger().info(f"원뿔 스캔 시작: {target_name}")
