...
class AdaptiveGripper:
    ...
    def get_status(self):
        ...
        # status = format(raw, "016b")

        # print("raw:", raw)
        # print("binary:", status)
        # print("grip bit:", status[-2])
...
class TargetScanNode(Node):
        ...
        for target_name in target_names:
            ...
            while not find_target:
                ...
                if grip_response.is_find:
                    ...
                    target_coordinate[:3] = grip_response.coordinate[:3]
                    target_coordinate[2] -= 45
                    ######################################
                    # +ADD
                    ...
                    # 자세 표현만 -180°~180°로 정리
                    target_coordinate[5] = (target_coordinate[5] + 180.0) % 360.0 - 180.0
                    # +ADD
                    ######################################
    ...
    def move_to_position(self, move_name, target_position):
        ...
        try:
            ret = movel(target_position, vel=VELOCITY, acc=ACCELERATION)
            if ret != 0:
                self.report_error(f"{move_name} 이동 실패")
                return False       
