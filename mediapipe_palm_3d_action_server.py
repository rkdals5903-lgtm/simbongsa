...
- T_gripper2camera.npy는 gripper -> camera 변환으로 해석한다.
- 카메라 점을 gripper 좌표로 변환할 때 역행렬을 사용한다.
- 기본 calibration_frame은 gripper_tcp이다.
"""
...
class MediaPipePalm3DActionServer(Node):
    ...
    def _load_transform(self) -> np.ndarray:
        path = Path(self.transform_path).expanduser().resolve()
        if not path.is_file():
    ...
    async def execute_find_hand(self, goal_handle) -> FindOrder.Result:
        ...
        try:
            while rclpy.ok():
                ...
                if (
                    ...
                    coordinate = self.latest_base_xyz_mm.copy()
                    goal_handle.succeed()
                    result = FindOrder.Result()
                    result.found = True
                    result.coordinate = [float(value) for value in coordinate]
                    result.message = "hand detected"
                    self.get_logger().info(
                        "Hand detected [mm]: "
                        f"[{coordinate[0]:.1f}, {coordinate[1]:.1f}, "
                        f"{coordinate[2]:.1f}]"
                    )
                    return result
    ...
    def _camera_to_base_mm(
        ...
        base_gripper_translation_mm = np.array(
            [
        ...
        camera_point_h = np.array(
            [
                float(camera_xyz_mm[0]),
                float(camera_xyz_mm[1]),
                float(camera_xyz_mm[2]),
                1.0,
            ],
            dtype=np.float64,
        )
        gripper_point_h = self.T_calibration_camera_mm @ camera_point_h
        gripper_point_mm = gripper_point_h[:3] / float(gripper_point_h[3])
        return rotation @ gripper_point_mm + base_gripper_translation_mm
